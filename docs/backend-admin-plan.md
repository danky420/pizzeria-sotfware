...
- **`customer/` — an unplanned but reusable head start.** A React/Vite app already exists with a menu renderer, cart, product sheet, and hours logic (`App.tsx`, `components/{CartSheet,ProductSheet,MenuSections}.tsx`, `lib/{cart,menu,hours,format}.ts`, `api/{client,types}.ts`) and a built `dist/`. It was written without the real backend order-submit contract in hand (built in parallel with, not after, `backend/`), so **it must be audited against `backend/src/schemas/orders.ts` (or wherever the public order-submit Zod schema actually lives) and `backend/src/routes/public/orders.ts` before being trusted** — check required fields (phone), payload shape, response shape, and error-code handling line by line rather than assuming it already matches.
...
Already substantially built (see "Current build state") — the remaining work is an audit and finish pass, not a rewrite:
