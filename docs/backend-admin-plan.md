...
5. **Retire the static site** per "Retiring the static site" above, and rewrite `CLAUDE.md`/`READER.md` to describe the four-piece architecture (this repo doc is the detailed reference; the root docs should point here rather than duplicate it, same convention as today). Also fix `backend/src/app.ts`'s static-file serving to match the plan's path prefixes: it currently serves `admin/dist` at `/` and nothing serves `employee/dist` at all — move to `/` → `customer/dist`, `/admin` → `admin/dist` (with its `VITE_EMPLOYEE_APP_URL` default and internal routing still working from that prefix), `/staff` → `employee/dist`, each with an SPA fallback for client-side routing, `/api/*` unchanged. `admin/vite.config.ts` and `employee/vite.config.ts` may need a `base` path adjustment to match (check whether `employee/vite.config.ts` already sets `base:"/staff/"` on build — a prior step reported it does; `admin/` likely still assumes root and needs the same treatment for `/admin`).
