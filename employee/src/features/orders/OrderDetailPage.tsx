...
/**
 * `admin/`'s detail page, minus the link into customer history — that section
 * of the back office does not exist here, and the API would refuse a STAFF
 * session on it anyway. Everything a counter actually needs (who, where, what,
 * and the next status) is on this screen.
 */
export function OrderDetailPage() {
  ...
  const setStatus = useMutation({
    ...
    onSuccess: (result) => {
      ...
      // The queue is a separate query and is what everyone looks at next, so it
      // must not show the old status for up to 12 seconds after the change.
  ...
  return (
    <>
      ...
      <Panel
        title="Estado"
        actions={
          <Badge tone={orderStatusTone(data.status)}>{ORDER_STATUS_LABELS[data.status]}</Badge>
        }
      >
